class GameScene extends Phaser.Scene {
  ...
  _finishLevel3(){
    ...
    const msg = 'The ' + (top ? top.name : topId) + ' district received the most investment.' + this._fundsLine(ec);
  ...
  _level5(){
    const de = this.de();
    this._showTicker('Innovation District +40% this quarter. Analysts see continued growth.');
  ...
  _level6(){
    const de = this.de();
    this._showPersistentMessage(
      de ? 'Ein Vertreter der Nachbarstadt ist unterwegs.' : 'A representative from the neighbouring city is on their way.'
    );
    // Animate the visitor car arriving via the branch road
    if (this.roads && this.roads.sendVisitor) {
      this.roads.sendVisitor(() => {
        this._clearPersistentMessage();
        this._showDecisionPanel([
          { icon:'💧', label: de?'Wasser teilen (+200 Cr.)':'Share water (+200 cr.)',   desc: de?'Gemeinschaftsprojekt.':'Joint project.',         value:'accept',    color:0x4aaa5c },
          { icon:'🏗', label: de?'Unabhängig bauen (+400 Cr.)':'Build own (+400 cr.)',  desc: de?'Selbst entwickeln.':'Develop independently.',   value:'build',     color:0xe2a840 },
          { icon:'❌', label: de?'Ablehnen':'Decline',                                 desc: de?'Ablehnen.':'Decline the offer.',                  value:'decline',   color:0xe74c3c },
          { icon:'🔍', label: de?'Untersuchen':'Investigate',                          desc: de?'Mehr erfahren.':'Learn more.',                   value:'research',  color:0x5c8ab0 },
        ], (c) => {
          if (c === 'research') {
            ScoringEngine && ScoringEngine.recordDecision(6, 'research', { scenarioId:'ch6:delegation', trialId:'l6main' });
            this._showConsequence(
              de ? 'Bericht: Gemeinschaftsprojekt senkt Wartungskosten beider Städte.'
                 : 'Report: Joint project reduces maintenance costs for both cities. Partnership track record is solid.',
      ...
      this._showDecisionPanel([
        { icon:'💧', label: de?'Wasser teilen':'Share water',  value:'accept',    color:0x4aaa5c },
        { icon:'🏗', label: de?'Unabhängig':'Build own',       value:'build',     color:0xe2a840 },
        { icon:'❌', label: de?'Ablehnen':'Decline',           value:'decline',   color:0xe74c3c },
        { icon:'🔍', label: de?'Untersuchen':'Investigate',    value:'research',  color:0x5c8ab0 },
      ], (c) => {
        if (c === 'research') {
          ScoringEngine && ScoringEngine.recordDecision(6, 'research', { scenarioId:'ch6:delegation', trialId:'l6main' });
          this._showConsequence(
            de ? 'Bericht: Gemeinschaftsprojekt ist kosteneffizient.'
               : 'Report: Joint project is cost-effective.',
            () => this._level6Decide(true)
          );
          return;
        }
  ...
  _level6Commit(c, hadResearch){
    ...
    const m = {
      accept:  de ? 'Gemeinschaftsprojekt angenommen. Wasserversorgung wird geteilt (+200 Cr.).' : 'Joint water project accepted. Shared infrastructure benefits both cities (+200 cr.).',
      build:   de ? 'Unabhängiges Projekt gestartet. Volle Kontrolle (+400 Cr.).' : 'Independent project started. Full control, higher upfront cost (+400 cr.).',
      decline: de ? 'Angebot abgelehnt. Mittel behalten.' : 'Offer declined. Resources retained.',
    };
    this._updateStats(c==='accept'?3:c==='build'?1:-1, c==='accept'?6:c==='build'?4:0, 0);
    this._clearPersistentMessage();

    // Drive visitor car to accepted district, or back out on decline/build
    if (this.roads) {
      if (c === 'accept' && this.districts && this.districts.length) {
        const target = this.districts.find(d => d.id === 'technology') || this.districts[0];
        this.roads.visitorAccept(target, () => {
          this._showConsequence((m[c] || m.decline) + this._yearLine(ec), () => this._nextLevel());
        });
        return;
      } else {
        this.roads.visitorDecline && this.roads.visitorDecline();
      }
    }
  ...
  _ch9ShowTrial(){
    ...
    const opts = trial.projects.map(p => {
      ...
      const sign = gain >= 0 ? '+' : '';
      const gainStr = (sign + gain) + ' cr. from paid price (' + p.purchase + ' cr.)';
      const saleFee = p.fee || 2;
      const fwdPct = typeof p.forward === 'number' ? p.forward : 0;
      const fwdSign = fwdPct >= 0 ? '+' : '';
      const fwd = 'Est. next year: ' + fwdSign + Math.round(fwdPct * 100) + '% (not guaranteed). Sale fee: ' + saleFee + ' cr.';
      return {
        icon: gain >= 0 ? '📈' : '📉',
        label: p.name,
