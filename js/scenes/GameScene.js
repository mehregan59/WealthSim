class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.currentLevel = 0;
    this.cubes = []; this.decisionPanel = null; this.worldBtn = null;
    ...
    this.time.delayedCall(3000,()=>{
      ...
      this.time.delayedCall(1800,()=>{
        ...
        this._showDecisionPanel([
          ...
          const e=eff[choice]||eff.continue;
          this._updateStats(...e.delta);
  ...
  _level3_expansion() {
    ...
    this.time.delayedCall(3000,()=>{this._spawnResourceCubes(6);this._showPersistentMessage('The city receives 600 new credits.\nDrag the glowing cubes into your chosen districts.');});
  }
  ...
  _onResourceDropped(district,value) {
    this.cubes=this.cubes.filter(c=>{try{return c.container&&!c.container.destroyed;}catch(e){return false;}});
    this._updateStats(2,4,-3);
    if(this.cubes.length===0&&this.currentLevel===3){this._clearPersistentMessage();this.time.delayedCall(1000,()=>this._level3_outcome());}
  }
  ...
  _level4_todayOrTomorrow() {
    ...
    this.time.delayedCall(3000,()=>{
      ...
      this._showDecisionPanel([
        ...
        {icon:'\uD83C\uDF93',label:'Research University',desc:'No reward yet.\nPowerful several levels later.',value:'university',color:0x4ecdc4}
      ],(choice)=>{
  ...
  _level5_boom() {
    ...
    this.time.delayedCall(3000,()=>{
      ...
      tech.receiveResource(4);tech.receiveResource(3);
    ...
    this.time.delayedCall(3000,()=>{
      ...
      this.time.delayedCall(2000,()=>{
        ...
        this._showDecisionPanel([
          ...
          {icon:'\u2B07',label:'Reduce partially',desc:'Cautious middle path.',value:'reduce',color:0xe2a840},
  ...
  _showConsequence(text,onContinue){
    ...
    const W=this.W,pw=Math.min(620,W-80),ph=90,px=(W-pw)/2,py=this.H-165;
    ...
    const txt=this.add.text(W/2,py+45,text,{fontFamily:'Georgia,serif',fontSize:14,color:'#c8d8e8',align:'center',wordWrap:{width:pw-40}}).setOrigin(0.5).setDepth(63);
    ...
    this.time.delayedCall(1000,()=>{
      const label=currentLang==='de'?'Weiter \u2192':'Continue \u2192';
      this.worldBtn=new WorldButton(this,W/2,this.H-230,label,()=>{this.worldBtn=null;if(onContinue)onContinue();});
    });
  }
  ...
  _showDecisionPanel(options,callback){
    ...
    const panelW=cols*btnW+(cols-1)*10+40,panelH=112;
    ...
    options.forEach((opt,i)=>{
      ...
      const draw=(hover)=>{btnBg.clear();btnBg.fillStyle(opt.color,hover?0.38:0.14);btnBg.fillRoundedRect(bx,by,btnW,88,8);btnBg.lineStyle(hover?2:1,opt.color,hover?0.9:0.4);btnBg.strokeRoundedRect(bx,by,btnW,88,8);};
      ...
      const hit=this.add.rectangle(bx+btnW/2,by+44,btnW-4,86,0xffffff,0).setInteractive({useHandCursor:true});
